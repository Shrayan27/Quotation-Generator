...
const logoPath = path.join(process.cwd(), "kb_logo.png");
let kbLogoBase64 = "";
try {
...
export function generateQuotationHtml(q: any): string {
	...
	const itemRowsHtml = items
      ...
      <tr>
        ...
        ${
					q.isDocComposite
						? `<td style="padding:8px;border:1px solid #c8c8c8;text-align:right;font-size:12px;vertical-align:top;" colspan="3">${qty}</td>`
						: `
    ...
    <tr style="background:#fcfdfc;font-weight:800;color:#1a3a1a;">
      <td colspan="5" style="padding:10px 12px;border:1px solid #c8c8c8;text-align:left;font-size:12px;text-transform:uppercase;letter-spacing:0.5px;">
  ...
  <div class="page">
    ...
    <div class="header-bar">
      ...
      <div style="background:#fff;color:#2c2a28;padding:12px calc(20px + 8mm) 8px 45px;width:max-content;flex-shrink:0;box-sizing:border-box;clip-path:polygon(40px 0, 100% 0, 100% 100%, 0 100%);font-size:10px;font-weight:700;position:relative;">
        <div style="position:absolute;top:0;left:30px;right:31px;height:5px;background:#facc04;"></div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:0;">
      <thead>
        <tr style="background:#f4f6f4;color:#1a3a1a;">
          ...
          <th style="padding:8px;border:1px solid #c8c8c8;text-align:right;width:70px;" ${q.isDocComposite ? 'colspan="3"' : ""}>Qty</th>
          ${!q.isDocComposite ? `
