...
export const TrackerPanel: React.FC<TrackerPanelProps> = ({
	...
	const loadNotifications = async () => {
		setLoading(true);
		try {
			const activeSequences = await api.getActiveFollowUps();
			// Map backend structure to frontend structure
			const mapped = activeSequences.map((seq: any) => ({
				id: seq.id,
				custName: seq.quotation?.billName || "Unknown Customer",
				custEmail: seq.customerEmail,
				quoteNo: seq.quotation?.quoteNumber || "Unknown Quote",
				quoteDate: seq.quotation?.quoteDate || new Date().toISOString(),
				dueDate: seq.nextFollowUpDate,
				status: seq.status === "STOPPED" ? "replied" : "pending",
				followUpCount: seq.followUpCount,
			}));
	...
	const handleMarkReplied = async (id: string) => {
		try {
			await api.markFollowUpReplied(id);
			setNotifications((prev) =>
				prev.map((n) =>
					n.id === id ? { ...n, status: "replied" as const } : n,
				),
			);
			setToast("Follow-up sequence marked as stopped.");
		} catch {
			setToast("Failed to mark follow-up as replied.", "warn");
		}
	};
	...
	const handleDelete = async (id: string) => {
		try {
			...
			setToast("Expunged follow-up sequence.");
		} catch {
			setToast("Failed to delete follow-up sequence.", "warn");
		}
	};
	...
	return (
		<div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
			...
			<div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col z-10 border-l border-gray-200">
					...
					<div className="flex items-center gap-2.5">
						...
						<div>
							...
							<p className="text-[11px] text-brand-200">
								Local notification schedule parameters
							</p>
						</div>
					</div>
					<button
				...
				{/* Manual trigger testing row */}
				<div className="px-6 py-3 bg-brand-50/50 border-b border-brand-100 flex flex-col gap-2">
					<div className="text-[11px] font-bold text-brand-900 uppercase tracking-wider">
						Outreach Engine testing
					</div>
					<div className="flex gap-2">
						<button
							onClick={async () => {
								try {
									setLoading(true);
									await api.triggerFollowUpCheck(false);
									setToast("Follow-up check executed successfully.");
									await loadNotifications();
								} catch {
									setToast("Failed to run follow-up check.", "warn");
								} finally {
									setLoading(false);
								}
							}}
							disabled={loading}
							className="flex-1 py-1.5 bg-brand-800 hover:bg-brand-950 disabled:opacity-50 text-white rounded-lg text-xs font-bold transition-all shadow-sm">
							Run Normal Check
						</button>
						<button
							onClick={async () => {
								try {
									setLoading(true);
									await api.triggerFollowUpCheck(true);
									setToast("Outreach forced! Due emails dispatched.");
									await loadNotifications();
								} catch {
									setToast("Failed to force follow-up check.", "warn");
								} finally {
									setLoading(false);
								}
							}}
							disabled={loading}
							className="flex-1 py-1.5 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-brand-950 rounded-lg text-xs font-black transition-all shadow-sm">
							⚡ Test Force Send
						</button>
					</div>
				</div>
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-3 bg-[#fbfdfb]">
					{loading ? (
						<div className="py-12 text-center text-xs font-semibold text-gray-400">
							Loading active outreach sequences...
						</div>
						...
						<div className="py-16 text-center">
							...
							<p className="text-[11px] text-gray-400 mt-1 max-w-xs mx-auto">
								Whenever you save a commercial quotation, the AI outreach
								sequence will track it here.
							</p>
						</div>
					) : (
						notifications.map((n) => {
								...
								<div
									...
									<div className="text-xs text-gray-500 font-sans space-y-0.5 mb-3">
										...
										<div className="text-[11px] text-gray-400 flex items-center gap-1">
											<Clock className="w-3 h-3" /> Emails Sent:{" "}
											{n.followUpCount}
										</div>
									</div>
